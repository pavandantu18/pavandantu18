<!-- =========================
     HERO SECTION
========================= -->

<h1 align="center">Hey 👋 I'm Pavan Kumar Reddy Dantu</h1>
<h3 align="center">🚀 Full Stack Developer | Problem Solver | Builder</h3>

<p align="center">
  <a href="https://pavandantu-portfolio.vercel.app/" target="_blank">
    🌐 Portfolio
  </a>
  •
  <a href="mailto:pavankumarreddydantu@gmail.com">
    📫 Email
  </a>
  •
  <a href="https://www.linkedin.com/in/pavandantu18/" target="_blank">
    💼 LinkedIn
  </a>
</p>

---

<!-- =========================
     TROPHIES
========================= -->

<p align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=pavandantu18&theme=darkhub&no-frame=true&row=1&column=6" />
</p>

---

<!-- =========================
     ABOUT ME
========================= -->

## 🧠 About Me

- 💻 Full Stack Developer with a strong focus on **clean UI + scalable backend**
- 🔥 Love building **end-to-end applications**, not just demos
- ⚙️ Comfortable with **frontend, backend, DevOps, and cloud**
- 🚀 Always experimenting, learning, and shipping

> *I don’t just write code — I design flows, debug systems, and polish UX.*

---

<!-- =========================
     WHAT I'M BUILDING
========================= -->

## 🛠️ What I’m Working On

- Full-stack apps using **React + Node.js**
- REST APIs with proper architecture & validation
- Cloud-ready deployments (Docker, Kubernetes, AWS)
- Clean UI with modern CSS & component-based design

---

<!-- =========================
     TECH STACK
========================= -->

## ⚡ Tech Stack

### 🖥️ Frontend
- React, React Native
- JavaScript, TypeScript
- HTML5, CSS3, Sass, Tailwind CSS

### ⚙️ Backend
- Node.js, Express.js
- Java, Spring Boot
- REST APIs, Microservices

### 🗄️ Databases
- MongoDB
- MySQL
- PostgreSQL
- Oracle

### ☁️ DevOps & Cloud
- AWS
- Docker
- Kubernetes
- Jenkins
- Kafka
- RabbitMQ
- Redis
- Hadoop

### 🧰 Tools
- Git & GitHub
- CI/CD Pipelines
- Linux

---

<!-- =========================
     ICON GRID
========================= -->

<p align="center">
  <img src="https://skillicons.dev/icons?i=react,nodejs,java,spring,python,mongodb,mysql,postgres,aws,docker,kubernetes,jenkins,redis,kafka,typescript,tailwind,git" />
</p>

---

<!-- =========================
     PROJECTS
========================= -->

## 🚀 Featured Projects

### 📝 Fullstack Notes App
- React frontend + Node.js backend
- CRUD operations with REST APIs
- Modern UI with modals, toasts, and animations
- Deployed on Render

🔗 **Live Demo:**  
<a href="https://notes-app-a535.onrender.com/" target="_blank">
https://notes-app-a535.onrender.com/
</a>

---

<!-- =========================
     STATS
========================= -->

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=pavandantu18&show_icons=true&theme=tokyonight" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=pavandantu18&layout=compact&theme=tokyonight" />
</p>

---

<!-- =========================
     CONNECT
========================= -->

## 🤝 Let’s Connect

<p align="left">
  <a href="https://www.linkedin.com/in/pavandantu18/" target="_blank">
    <img src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/linked-in-alt.svg" height="30" width="40" />
  </a>
</p>

---

<!-- =========================
     FOOTER
========================= -->

<p align="center">
  🚀 Building • Learning • Shipping  
</p>
